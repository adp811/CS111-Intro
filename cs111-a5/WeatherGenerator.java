...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        double firstDayProb = StdRandom.uniform();
        ...
        for(int i = 0; i < drywet.length; i ++){
            if(drywet[i][0] == longitute && drywet[i][1] == latitude){
                ...
                //StdOut.println(dryWetProb);
            }
        }
        for(int i = 0; i < wetwet.length; i ++){
            if(wetwet[i][0] == longitute && wetwet[i][1] == latitude){
                wetWetProb = wetwet[i][month];
                //StdOut.println(wetWetProb);
            }
        }

        if(firstDayProb <= 0.49) {
         wetDay = true;
         forecast[0] = 1;
        } else if(firstDayProb >= 0.50 && firstDayProb <= 0.99){
         wetDay = false;
         forecast[0] = 2;
        }

        //StdOut.println("First Day is: " + wetDay + " prob: " + firstDayProb + " " + forecast[0]);

        for(int i = 1; i < forecast.length; i++){
            
            if (wetDay == true){
                double random = StdRandom.uniform();
                if (random <= wetWetProb){
                    wetDay = true;
                } else {
                    wetDay = false;
                }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        // {1, 2, 2, 2, 1, 2, 1, 2, 1, 2, 2, 2, 2, 2, 1}

        for(int i = 0; i < forecast.length - 1; i ++){
            
            if(forecast[i + 1] == mode && forecast[i] == mode){
                spellCount ++;
                if (spellCount > maxSpellCount) maxSpellCount = spellCount;
            } else { 
                spellCount = 0;
            }   
        
        }
        return maxSpellCount + 1; 
    }
    ...
    public static void main (String[] args) {
        ...
        //StdRandom.setSeed(1604120804573L);
